...
class SAC2Agent(parl.Agent):
    def __init__(self, algorithm, obs_dim, act_dim, low_act_dim, high_act_dim):
        assert isinstance(obs_dim, int)
        assert isinstance(act_dim, int)
        self.obs_dim = obs_dim
        self.act_dim = act_dim
        ...
        super(SAC2Agent, self).__init__(algorithm)
    ...
    def build_program(self):
        ...
        self.learn_program = fluid.Program()
        self.sample_program = fluid.Program()
        self.exp_fast_learn_program = fluid.Program()
        self.exp_fast_learn_double_program = fluid.Program()
        self.get_trend_program = fluid.Program()
        self.get_ampl_program = fluid.Program()

        with fluid.program_guard(self.pred_program):
            obs = layers.data(
                name='obs', shape=[self.obs_dim], dtype='float32')
            last_act = layers.data(
                name='last_act', shape=[self.high_act_dim], dtype='float32')
            # print("[LD ADAPTER][SAC AGENT] obs = ", obs)
            # print("[LD ADAPTER][SAC AGENT] last_act = ", last_act)
            self.pred_act = self.alg.predict(obs, last_act)
        
        with fluid.program_guard(self.get_trend_program):
            obs = layers.data(
                name='obs', shape=[self.obs_dim], dtype='float32')
            last_act = layers.data(
                name='last_act', shape=[self.high_act_dim], dtype='float32')
            # print("[LD ADAPTER][SAC AGENT] obs = ", obs)
            # print("[LD ADAPTER][SAC AGENT] last_act = ", last_act)
            self.trend = self.alg.get_only_trend(obs, last_act)
        
        
        
        with fluid.program_guard(self.get_ampl_program):
            obs = layers.data(
                name='obs', shape=[self.obs_dim], dtype='float32')
            last_act = layers.data(
                name='last_act', shape=[self.high_act_dim], dtype='float32')
            # print("[LD ADAPTER][SAC AGENT] obs = ", obs)
            # print("[LD ADAPTER][SAC AGENT] last_act = ", last_act)
            self.ampl = self.alg.get_only_ampl(obs, last_act)
        ...
        with fluid.program_guard(self.sample_program):
            ...
            last_act = layers.data(
                name='last_act', shape=[self.high_act_dim], dtype='float32')
            self.sample_act, _ = self.alg.sample(obs, last_act)
        ...
        with fluid.program_guard(self.learn_program):
            ...
            last_act = layers.data(
                name='last_act', shape=[self.high_act_dim], dtype='float32')
            self.critic_cost, self.actor_cost, self.alpha_cost = self.alg.learn(obs, act, reward,
                                                                                next_obs, terminal, last_act)
            
        with fluid.program_guard(self.exp_fast_learn_program):
            obs = layers.data(
                name='obs', shape=[self.obs_dim], dtype='float32')
            act = layers.data(
                name='act', shape=[self.act_dim], dtype='float32')
            ...
            last_act = layers.data(
                name='last_act', shape=[self.high_act_dim], dtype='float32')
            exp_trend = layers.data(
                name='exp_trend', shape=[self.high_act_dim], dtype='float32')
            A = layers.data(
                name='A', shape=[self.high_act_dim, self.low_act_dim], dtype='float32')
            self.exp_fast_cost = self.alg.exp_fast_learn(obs, last_act, exp_trend, A)
        
        with fluid.program_guard(self.exp_fast_learn_double_program):
            obs = layers.data(
                name='obs', shape=[self.obs_dim], dtype='float32')
            act = layers.data(
                name='act', shape=[self.act_dim], dtype='float32')
            ...
            A = layers.data(
                name='A', shape=[self.high_act_dim, self.low_act_dim], dtype='float32')
            self.exp_fast_cost_double = self.alg.exp_fast_learn_double(obs, last_act, exp_trend, A)

    def predict(self, obs, last_act):
        obs = np.expand_dims(obs, axis=0)
        feed = {
            'obs': obs,
            'last_act': last_act
    ...
    def get_trend(self, obs, last_act):
        obs = np.expand_dims(obs, axis=0)
        feed = {
            'obs': obs,
            'last_act': last_act
        ...
        # print('===obs===:', obs)
        trend = self.fluid_executor.run(
            self.get_trend_program, feed=feed,
            fetch_list=[self.trend])[0]
        # print(act)
        trend = np.squeeze(trend)
        return trend

    def get_ampl(self, obs, last_act):
        obs = np.expand_dims(obs, axis=0)
        feed = {
            'obs': obs,
            'last_act': last_act
        ...
        # print('===obs===:', obs)
        ampl = self.fluid_executor.run(
            self.get_ampl_program, feed=feed,
            fetch_list=[self.ampl])[0]
        # print(act)
        ampl = np.squeeze(ampl)
        return ampl

    def sample(self, obs, last_act):
        obs = np.expand_dims(obs, axis=0)
    ...
    def learn(self, obs, act, reward, next_obs, terminal, last_act):
        # self.learn_it += 1
        # feed = {
        #     'obs': obs,
        #     'act': act,
        ...
        [critic_cost, actor_cost, alpha_cost] = self.fluid_executor.run(
            self.learn_program,
            feed=feed,
            fetch_list=[self.critic_cost, self.actor_cost, self.alpha_cost])
        # print('critic loss = ', critic_cost[0])
    ...
    def exp_fast_learn(self, obs, last_act, exp_trend, A):
        self.learn_it += 1
        feed = {
            'obs': obs,
            'last_act': last_act,
        ...
        [exp_fast_cost] = self.fluid_executor.run(
            self.exp_fast_learn_program,
            feed=feed,
            fetch_list=[self.exp_fast_cost])
        return exp_fast_cost
    
    def exp_fast_learn_double(self, obs, last_act, exp_trend, A):
        self.learn_it += 1
        feed = {
            'obs': obs,
            'last_act': last_act,
        ...
        [exp_fast_cost_double] = self.fluid_executor.run(
            self.exp_fast_learn_double_program,
            feed=feed,
            fetch_list=[self.exp_fast_cost_double])
        return exp_fast_cost_double
    ...
    def normalizer(self, obs, rpm):
        ...
        mean, std = rpm.countRes(rpm.getStates())
        obs = np.array(obs)
        obs_new = (obs - mean) / std
        # print("[NORMALIZE DEBUG] obs_new before correct = ", obs_new)
        for i in range(obs.shape[0]):
            if i%16==14 or i%16==15:
                obs_new[i] = obs[i]
        # print("[NORMALIZE DEBUG] obs_new after correct = ", obs_new)
        # print("mean:", mean)
        # print("std:", std)
        # print("obs:", obs)
        return obs_new
