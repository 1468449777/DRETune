...
from maEnv import change_ma_json
import random
import os
import threading
...
class NodesEnv():
    def __init__(self):
        ...
        # 低维空间投影
        self.ld_adaptor = None
        self.ld_adaptor_double = None
        self.init_state_for_transfer = []
        ...
        self.last_action_double=None

        # step用，计算无用step
        self.unuse_step = 0
        self.human_exp_hitcnt = 0
        self.exp_fast_learn_hit_cnt = 0
    ...
    def init_nodes(self):
        ...
        self.state_dim = 16 * (self.ce_num + self.se_num)  # TODO：增加当前缓冲池大小作为状态之一
        # 动作变量的维度
        self.action_dim = len(self.all_actions)
        print('all_actions = ', self.all_actions)

        change_ma_json.change_ma_json()
    ...
    def get_all_nodes_state(self):
        ...
        for se in self.se_info:
            ...
            current_nodes_state_list.append(current_state_se)
            if (self.expert_exp == True and (self.state == 1 or self.mode == 2)) or self.use_exp_fl:
        ...
        for ce in self.ce_info:
            ...
            current_nodes_state_list.append(current_state_ce)
            if (self.expert_exp == True and (self.state == 1 or self.mode == 2)) or self.use_exp_fl:
                datautils.status_to_labels(ce)

        print("The current_state is : ", current_nodes_state)
        return current_nodes_state, current_nodes_state_list
    ...
    def print_all_nodes(self):
        ...
        for ce in self.ce_info:
            if ce.is_primary == True:
                print('Node {}{}: hit_ratio = {}, bp_size = {}, qps = {}'.format(ce.name, ce.uuid, ce.last_hr, ce.last_bp_size, q_after_total))
            else:
    ...
    def step(self, action):
        ...
        # 按节点划分real_action
        print("[MERGE DEBUG] step action shape = ", action.shape)
        ...
        # print("real_action = {0}".format(real_action))
        # print("============================= [LHS LOG] =============================")
        ...
        # all ce qps
        parallel_qps_list = [0 for i in range(len(self.ce_info))]
        ...
        try:
            ...
            def paralell_get_nodes_qps_base(parallel_qps_list, node, i):
                q_after_node = utils.get_node_qps(node)
                # parallel_qps_list.append(q_after_node)
                parallel_qps_list[i] = q_after_node
            ...
            for ce in self.ce_info:
                ...
                if ce.is_primary == True:
                    ...
                    for i in range(globalValue.CE_LOAD_NUM):
                        t = threading.Thread(target = paralell_get_nodes_qps_base, args=(parallel_qps_list, self.ce_info[i], i))
                ...
                # all ce qps
                q_after_ce += parallel_qps_list[qps_node_cnt]
                qps_node_cnt += 1
                ...
                q_before_total += ce.last_qps
                ce.last_qps = q_after_ce
                
                ce.last_action = real_action[cnt]
                cnt += 1
            # 获取当前状态,注意对于send_msg_to_server接收回的数据要做相应处理
            current_state, current_state_list = self.get_all_nodes_state()
            
            # 组合成新的S [14个状态，delta_qps，delta_hr]
            # 其中 delta_qps 对于se和从ce来说都是 0，对于主ce来说是实际值
            print("[MERGE DEBUG] current_state_list = ", current_state_list)
            node_state_cnt = 0
            new_current_state = []
            for se in self.se_info:
                old_node_state = current_state_list[node_state_cnt]
                new_node_state = old_node_state
                new_node_state.append(se.delta_q0)
                new_node_state.append(se.delta_h0)
                new_current_state += new_node_state
                node_state_cnt += 1
                
            for ce in self.ce_info:
                old_node_state = current_state_list[node_state_cnt]
                new_node_state = old_node_state
                new_node_state.append(ce.delta_q0)
                new_node_state.append(ce.delta_h0)
                new_current_state += new_node_state
                node_state_cnt += 1
            print("[MERGE DEBUG] new_current_state = ", new_current_state)
        ...
        if self.init_bps_all >= present_bps_all:
            reward_bps = (self.init_bps_all - present_bps_all) / max(self.init_bps_all, present_bps_all)
        else:
            reward_bps = (self.init_bps_all - present_bps_all) / max(self.init_bps_all, present_bps_all)
        ...
        q_after_total = 0
        q_t0_total = 0
        for i in range(len(self.ce_info)):
            q_after_total += parallel_qps_list[i]
            q_t0_total += self.ce_info[i].qps_t0
            # q_before_total += self.ce_info[i].last_qps
        delta_q0 = (q_after_total - q_t0_total) / q_t0_total
        delta_qt_total = (q_after_total - q_before_total) / q_before_total
        
        reward_qps = [utils.cal_r(delta_q0, delta_qt_total)]
        print("[ALL CE BPS] q_after_total = ", q_after_total)
        print("[ALL CE BPS] q_before_total = ", q_before_total)
        print("[ALL CE BPS] q_t0_total = ", q_t0_total)
        print("[ALL CE BPS] delta_q0 = ", delta_q0)
        print("[ALL CE BPS] delta_qt_total = ", delta_qt_total)
        print("[ALL CE BPS] reward_qps = ", reward_qps)
        ...
        if flag == True:
            ...
            for ce in self.ce_info:
                if ce.is_primary == True:
                    ...
                    ce.qps_before = parallel_qps_list[tmp_node_cnt]
        ...
        # 一个文件内记录多个节点的值
        with open("./test_model/qps_store.txt", "a+", encoding="utf-8") as f_qps:
            f_qps.write("cluster@qps=" + str(q_after_total) + "$")
            for ce in self.ce_info:
                # if ce.is_primary == True:
        ...
        time.sleep(buffer_time)
        self.exit_all_nodes()
        if self.ld_adaptor is None:
            print("[LD ADAPTER]00000000000000000000000000000000000000000000000000000")
            self.all_last_action_2 = self.all_last_action
    ...
    def reset(self):
        ...
        print("env.all_last_action = ", list(self.all_last_action))
        ces_send = []
        ses_send = []
        bps_all = []
        ...
        try:
            ...
            for ce in self.ce_info:
                ...
                # all ce qps
                print("[ALL CE BPS] bps list = ", parallel_qps_list)
                print("[ALL CE BPS] qps_node_cnt = ", qps_node_cnt)
                print("[ALL CE BPS] parallel_qps_list[qps_node_cnt] = ", parallel_qps_list[qps_node_cnt])
                q_after_ce += parallel_qps_list[qps_node_cnt]
                qps_node_cnt += 1
                ...
                ce.last_action = -2
                cnt += 1
            print("[DEBUG][reset] enter get_all_nodes_state()")
            current_state, current_state_list = self.get_all_nodes_state()
            ...
            print("[MERGE DEBUG] new_current_state = ", new_current_state)
            current_state = new_current_state
            if len(self.init_state_for_transfer) == 0:
                self.init_state_for_transfer = current_state
